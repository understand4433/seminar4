...
Откат к конкретному коммиту. Если мы хотим откатить изменения до конкретного коммита, мы можем использовать команду git revert или git reset с определенным хеш-кодом коммита. Например, чтобы откатить изменения до коммита с хеш-кодом "123abc", выполните следующую команду:
```
git revert 123abc
...
Если у нас есть файлы, которые мы не хотим сохранять в Git (например, файлы с временными данными или базы данных), мы можем добавить их в файл ".gitignore". Файл ".gitignore" указывает Git, какие файлы и папки игнорировать при коммите изменений в репозиторий.